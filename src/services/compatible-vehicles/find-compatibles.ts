...
export const compareStrategies = {
  ...
  compareModel: {
    comparator: (vendorModel: CompareModel, ebayModel: CompareModel): { modelMatchScore: number, modelPercentMatch: number } => {
      // check full models strings for match
      const _model     = HashMaker.ebayMake(vendorModel.raw);
      ...
      if (
        ...
        (Math.min(vendorModel.raw.length, ebayModel.raw.length) / Math.max(vendorModel.raw.length, ebayModel.raw.length) * 100)
      ) {
        return {
          modelMatchScore:   4,
          modelPercentMatch: (Math.min(vendorModel.raw.length, ebayModel.raw.length) / Math.max(vendorModel.raw.length,
            ebayModel.raw.length) * 100)
        };
      }

      // if no match, check word by word

      const maxHashLength = Math.max(ebayModel.hashes.length, vendorModel.hashes.length);

      const cleanEbayModel = ebayModel.hashes.filter(word => vendorModel.hashes.find(rmWord => word.includes(rmWord) || rmWord.includes(
        word)));
      const maxHash        = cleanEbayModel.length > vendorModel.hashes.length ? cleanEbayModel : vendorModel.hashes;
      let minHash          = cleanEbayModel.length > vendorModel.hashes.length ? vendorModel.hashes : cleanEbayModel;
  ...
  compareMake:  {
    comparator: (vendorMake, ebayMake, makesAliasesCache) => {
      const makeAlias = makesAliasesCache.find(el => el.ebay_brand_id === ebayMake && el.rm_brand_id === vendorMake);
      return (makeAlias || (ebayMake === vendorMake)) ? 2 : 0;
    }
  },
  // year match gives 1 point
  compareYear:  {
    comparator: (vendorYear: number, ebayYear: number) => {
      return vendorYear <= ebayYear ? 1 : 0;
    }
  }
};

// const EbayVehiclesCache = {
// 	_caches: {},
//
...
export async function findCompatibles(vehicle: { make: string, model: string, year: string }, ebayVehicles: Array<EbayVehicle>): Promise<Array<{ epid: string, make: string, model: string, year: number }>> {
  ...
  const compatibles      = ebayVehicles
    .filter(({ hashes: { make, model, year }, model_submodel }) => {
      ...
      const makeScore                              = compareStrategies.compareMake.comparator(vehicleMakeHash,
        make,
        MAKES_ALIASES);
    ...
    .map(({ make, model_submodel, year, epid }) => ({
      epid, make, model: model_submodel, year: +year
    }));
  return compatibles;
}
